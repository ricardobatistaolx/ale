...
function! ale#job#Start(command, options) abort
    ...
    let l:job_info.command = a:command
    let l:job_options = {}
    ...
    if has('nvim')
        ...
        let l:job = jobstart(l:job_info.command, l:job_options)
    else
        ...
        let l:job_info.command = has('win32')
        \   ?  'cmd /c ' . l:job_info.command
        \   : split(&shell) + split(&shellcmdflag) + [l:job_info.command]
        ...
        let l:job = job_start(l:job_info.command, l:job_options)
    endif
    ...
    if l:job_id
        ...
        let s:job_map[l:job_id] = l:job_options
    endif
